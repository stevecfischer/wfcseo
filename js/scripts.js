...
window.onload = function () {
    ...
    $('[data-action="ajax-updatemanualdata"]').click(function (e) {
        ...
        emptyView();
        loading();
        $('#property_dashboard').modal('hide');
        obj = $(".edit_table_data").serializeArray();
        $.ajax({
    ...
    $('[data-action="ajax-viewmetrics"]').click(function (e) {
        console.log('ddd');
        e.preventDefault();
        emptyView();
        loading();
        $('#viewmetrics').modal('hide');
        obj = $(".viewmetrics").serializeArray();
        $.ajax({
            url: "includes/ajax.php",
            type: "POST",
            data: obj,
    ...
    function ajax_right($this) {
        ...
        if ($this.data('names')) {
            var params = $this.data('names').split(',');
        }
        var obj = new Object();
        if (params) {
    ...
    $('.wfc-toolbar').on('click', '.pluspage', function (e) {
    ...
    $('#editor').on('click', '.deletepage', function (e) {
...
function refreshTiny() {
    tinymce.init({
        ...
        toolbar2: "print preview media | forecolor backcolor | wfc",
        image_advtab: true,
...
$(function () {
    $('.nav-wfc-properties').trigger('click');
    $('.wfc-tooltip').tooltip();
    ...
    $('.sidebar-menu-toggle').click(function () {
        if ($(this).hasClass('wfc-sidebar-collapsed')) {
            $('#sidebar,.main-content').removeAttr('style');
            $(this).removeAttr('style');
            $('.nav-wfc-properties-container,.nav-wfc-templates-container').removeAttr('style');
            $(this).removeClass('wfc-sidebar-collapsed').removeClass('glyphicon-chevron-right').addClass('glyphicon-chevron-left');
        } else {
            $('#sidebar').css('width', '0px');
            $('.nav-wfc-properties-container,.nav-wfc-templates-container').css('display', 'none');
            $(this).css('left', '0px');
            $(this).addClass('wfc-sidebar-collapsed').addClass('glyphicon-chevron-right').removeClass('glyphicon-chevron-left');
            $('.main-content').css('width', '90%');
        }
    });
    var get_active_prop = $.parseJSON($.cookie("property_in_progress"));
    if (get_active_prop !== null) {

        // console.log(get_active_prop);
        var the_prop = $('.wfc-web-property-list > li:eq(' + get_active_prop.prop_index + ')');
        the_prop.css('background-color', '#fb0200');
        //$('.wfc-web-property-list').scrollTo(the_prop, 800, {offset: -100});
    }
    $('.nav-wfc-properties-container a').on('click', function () {
        var property_in_progress = {
            'class': $(this).attr('class'),
            'prop_id': $(this).data('code'),
            'prop_index': $(this).closest('.wfc-web-property').index()
        };
        //console.log(property_in_progress);
        $.cookie('property_in_progress', JSON.stringify(property_in_progress));
    });
    $('.scroll-to-top').on('click', function () {
        console.log('ere');
        $('html,body').animate({scrollTop: 0}, 300);
    });
    $(document).on('click', '.wfc-web-property a.table-site-template, .wfc-web-property a.view-metrics', function () {
        var propertyId = $(this).data('code');
        $("#viewmetrics").find('input#code').val(propertyId);
        $("#property_dashboard").find('input#code').val(propertyId);
    })
});
