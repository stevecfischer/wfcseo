<?php
    ...
    class CoreReportingApiReference
    {
         ...
         */
        function getHtmlOutput( $tableId = NULL, $from, $to, $metric, $optParams = array() ){
            //$output = $this->getHTMLForm($tableId);
            /*if( isset($tableId) ){
                try{
                    $results = $this->queryCoreReportingApi( $tableId, $from, $to, $metric, $optParams );
                    $output  = $this->getRawData( $results );
                    //$output .= $this->getFormattedResults($results);
                    //$output.='<pre>'.print_r($this->getRawData($results),true).'</pre>';
...
HTML;
         ...
         */
        private function queryCoreReportingApi( $tableId, $from, $to, $metric, $optParams ){
...
HTML;
        ...
        /**
         * Returns an array with raw data.
         *
         * @param GaData $results The results from the Core Reporting API.
         *
         * @return array The raw results.
         */
        private function getRawData( $results ){
            $result = array();
            if( count( $results->getRows() ) > 0 ){
                foreach( $results->getColumnHeaders() as $header ){
                ...
                foreach( $results->getRows() as $row ){
                    foreach( $row as $cell ){
                        $result[$i][] = htmlspecialchars( $cell, ENT_NOQUOTES );
                    }
                    $i++;
                }
            } else {
